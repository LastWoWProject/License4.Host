...
{
    $configarray = array( "name" => "License.Host Licensing Manager", "description" => "License module for WHMCS.<br />more info <a href=\"https://license.host\" target=\"_blank\">https://license.host</a>", "version" => "1", "author" => "license.host", "language" => "english", "fields" => array(  ) );
...
function licensing_output($vars)
{
    ...
    if( !$action ) 
    {
        ...
        include("token.php");
        $token = ob_get_clean();
        $balance =  file_get_contents("https://api.license4.host/resellerapi/getblanace?token=$token");
        ...
        include("token.php");
        $token = ob_get_clean();
        $status = file_get_contents("https://api.license4.host/resellerapi/getstatus?token=$token");
        ...
        include("token.php");
        $token = ob_get_clean();
        $msg = file_get_contents("https://api.license4.host/resellerapi/getmsg?token=$token");
        ...
        echo "$msg";
                                      ...
                                      echo "                    </div>\n                </div>\n            </div>\n     <br><br>       <div class=\"col-sm-6\">\n                <div class=\"panel panel-default panel-accent-green\">\n                    <div class=\"panel-heading\">\n                        <h3 class=\"panel-title\">Total license list</h3>\n                    </div>\n                    <div class=\"panel-body\">\n         <a class=\"btn btn-default btn-block\" onclick=\"location.href='https://api.license4.host/resellerapi/getlist?token=$token'\">Click here to view licenses from API</a>       ";
 ...
 echo"       <style>
      ...
      echo "Reseller level : <aa>$level</aa>";
        ...
        $products = WHMCS\Product\Product::where("servertype", "=", "gblicensing")->get();
        foreach( $products as $product ) 
        {
            echo "<option value=\"" . $product->id . "\">" . $product->name . "</option>";
        }
        $addons = WHMCS\Product\Addon::where("module", "=", "gblicensing")->get();
    ...
    else
    {
        if( $action == "list" ) 
        {
            ...
            $result = select_query("tblproducts", "id,name", array( "servertype" => "gblicensing" ), "name", "ASC");
            while( $data = mysql_fetch_array($result) ) 
            {
                echo "<option value=\"" . $data["id"] . "\">" . $data["name"] . "</option>";
            }
           
            $addons = WHMCS\Product\Addon::where("module", "=", "gblicensing")->orderBy("name")->get();
            foreach( $addons as $addon ) 
            {
                echo "<option value=\"a" . $addon->id . "\">" . $addon->name . "</option>";
            }
